...
const createBeams = (beamsArray: Array<number>) => ({ beams: beamsArray.map(beam => ({ angle: beam, direction: DIRECTION.NONE, distance: null as number, leftMeasures: [] as Array<number>, rightMeasures: [] as Array<number> })) });
...
const getDistanceFrom = (leftMeasures: Array<number>, rightMeasures: Array<number>): number => {
	...
	if(isNullOrUndefined(leftDistance) || isNullOrUndefined(rightDistance)){
		return 0;
	}
